...
if (isset($_POST['save'])) {
	...
	$ff = fopen($files,'w+');
	fwrite($ff,$str);
	if (!@$link = mysqli_connect($conn,$_POST['db_host'],$_POST['db_username'],$_POST['db_password'])) {
		alert_href('数据库连接失败！请重新输入连接参数。','index.php');
	}else{
		if (!mysqli_select_db($conn,$_POST['db_name'])) {
			if (!mysqli_query($conn,'CREATE DATABASE '.$_POST['db_name'].'')) {
...
<link href="../MKCMS5.0/install/css/install.css" type="text/css" rel="stylesheet" />
<script type="text/javascript" src="../MKCMS5.0/install/js/jquery.min.js"></script>
<script type="text/javascript" src="../MKCMS5.0/install/js/common.js"></script>
