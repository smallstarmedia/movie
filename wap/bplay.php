...
function get_play($t0){
	$result = mysqli_query($conn,'select * from mkcms_player where id ='.$t0.'');
...
$result = mysqli_query($conn,'select * from mkcms_vod where d_id = '.$_GET['play'].' ');
if (!!$row = mysqli_fetch_array($result)) {
	$d_id = $row['d_id'];
	$d_name = $row['d_name'];
	$d_jifen = $row['d_jifen'];
...
$result1 = mysqli_query($conn,'select * from mkcms_vod_class where c_id='.$d_parent.' order by c_id asc');
...
<script type="text/javascript"> 
    ...
    //设置延时函数
    function adsUp(){    
        $("#shiping_box").html('<iframe id="video" src="<?php echo $mkcms_domain;?>jx/jx.php?url=<?php $result = mysqli_query($conn,'select * from mkcms_vod where d_id ='.$d_id.'');
	if (!!$row = mysqli_fetch_array($result)){
$d_scontent=explode("\r\n",$row['d_scontent']);
//print_r($d_scontent);
...
{	$d_scontent[$i]=explode('|',$d_scontent[$i]);
	...
	echo $d_scontent[0][1];
		...
		return '';
	};?><?php $result = mysqli_query($conn,'select * from mkcms_vod where d_id ='.$d_id.'');
	if (!!$row = mysqli_fetch_array($result)){
$d_scontent=explode("\r\n",$row['d_scontent']);
//print_r($d_scontent);
for($i=0;$i<count($d_scontent);$i++)
