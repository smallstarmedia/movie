...
<div class="col-xs-1 padding-0">
  <div class="stui-pannel_hd">
	 ...
	 <table border="0" width="100%">
			 ...
			 <td><strong><marquee scrollamount="5" direction="left" align="Middle" style="padding-right:20px;"><?php echo $mkcms_gonggao;?></marquee></strong></td>
			 </tr>
			     </table></li>
  </div> </div>
  </div>
...
$i=0;
if ($i<12){
$result = mysqli_query($conn,'select * from mkcms_vod where d_rec=0 order by d_id desc LIMIT 0,12');
while ($row = mysqli_fetch_array($result)){
$cc="./bplay.php?play=";
$dd="./bplay/";
if ($mkcms_wei==1){
...
<a class="stui-vodlist__thumb lazyload img-shadow" href="'.$ccb.''.$html.'" '.$ok.' title="'.$row['d_name'].'" alt="'.$row['d_name'].'" data-original="'.$row['d_picture'].'">
<span class="play hidden-xs"></span>
</a><div class="stui-vodlist__detail">
<h4 class="title text-overflow">
<a href="'.$ccb.''.$html.'" '.$ok.' title="'.$row['d_name'].'">'.$row['d_name'].'</a>
</h4>
...
<ul class="list w-newfigure-list g-clear">
  <?php $result = mysqli_query($conn,'select * from mkcms_vod where d_rec=1 order by d_id desc LIMIT 0,10');
		while ($row = mysqli_fetch_array($result)){
$cc="./bplay.php?play=";
$dd="./bplay/";
if ($mkcms_wei==1){
...
echo '<li title="'.$row['d_name'].'" class="w-newfigure w-newfigure-180x153"><a href="'.$ccb.''.$html.'" '.$ok.' class="js-link"><span class="play hidden-xs"></span>
			 <div class="w-newfigure-imglink g-playicon js-playicon"><img class="hotju" src="'.$row['d_picture'].'" alt="'.$row['d_name'].'"  />
...
<h3 class="title"><img src="<?php echo $mkcms_domain;?>moban/<?php echo $mkcms_bdyun;?>/img/icon_1.png"><a href="dongman.php?m=/dongman/list.php?rank=createtime&page=1">最新动漫</a></h3>
<ul class="nav nav-text pull-right hidden-sm hidden-xs">
...
<!--友链-->
<?php if($mkcms_yq==1){?>
<div style="margin-top:-18px"><?php echo get_ad(8)?></div> 
